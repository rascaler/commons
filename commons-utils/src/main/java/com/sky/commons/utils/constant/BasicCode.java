...
public class BasicCode implements Serializable {
    ...
    /* 状态===>>>状态说明映射文件 */
    public static final String BASE_PATH = "/basic-code.properties";
    // 扩展
    public static final String FILE_PATH = "/sub-code.properties";
    ...
    private static Logger logger = LoggerFactory.getLogger(BasicCode.class);

    static {
        logger.info("加载错误码资源文件");
        try {
            ecodeMessageMap.putAll(loadBasicPropertie());
            loadPropertie(BasicCode.class.getResourceAsStream(FILE_PATH));
        } catch (IOException e) {
            e.printStackTrace();
...
//		}
    ...
    private static Map<String, String> loadBasicPropertie() throws IOException {
        ...
        try (InputStream is = BasicCode.class.getResourceAsStream(BASE_PATH)) {
            p.load(is);
    ...
    protected static void setEcodeMessageMap(Map<String, String> map) {
        BasicCode.ecodeMessageMap.putAll(map);
    }

    public static Map<String, String> getEcodeMessageMap() {
        return BasicCode.ecodeMessageMap;
    }
    ...
    public void checkEcodeMessage() throws IllegalAccessException {
        ...
        for (String ecode : ecodes) {
            if (!BasicCode.getEcodeMessageMap().containsKey(ecode)) {
